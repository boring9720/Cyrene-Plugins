...
| 场景 | 纯上游宿主（ZIP 导入） | 打了 FC 补丁的宿主 |
|---|---|---|
| 工具注册、面板 UI、扫码登录、下载转码 | ✅ | ✅ |
| 聊天/工作模式点歌（Harness 原生带工具） | ✅ | ✅ |
| **语音通话中点歌**（通话走 FC 循环） | ❌ 上游通话不调工具 | ✅ |

> 语音通话点歌依赖宿主 `call-manager` 走 function calling——上游 v1.2.2 通话为"直接调 LLM 不带工具"，此为上游现状。补丁宿主见项目文档；上游若未来支持通话工具调用，补丁即可移除。
...
## v0.3.3 安全加固（审核后续建议落地）

- **播放器事件监听器泄漏修复**：`CH_PLAYER_EVENT` 监听改具名函数，`unbindPlayerEvents` 完整 `removeListener`——反复启停不再累积泄漏
- **ffmpeg 一键安装供应链锁定**：下载内容 SHA-256 硬编码校验（`e9fd5e71…49db1`，不符即拒并删除）；302 跳转目标受域名白名单约束（仅 npmmirror 系）
- **窗口权限收敛（Electron 最佳实践）**：播放器窗口与插件面板均改为 `contextIsolation: true + nodeIntegration: false + sandbox: true`，经受控 preload 桥（`player-preload.cjs` / `bili-preload.cjs`）暴露白名单接口——面板桥仅放行 `plugin:bili-music:` 前缀通道，页面即使被注入也无法触达宿主其他 IPC
...
- **与宿主网易云音乐工具并存**：宿主 `music_*` 工具未配置 OpenAPI 时必然失败，会让 LLM 误选。补丁宿主已禁用之；纯上游宿主建议在工具开关里手动关掉 `music_*` 系列
